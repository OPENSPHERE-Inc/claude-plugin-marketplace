...
- ベースブランチ: `{{base}}`
- レビュー範囲: `{{review_range}}`（ブランチ全体の差分が対象の場合は `(該当なし)`）
...
1. `creview:start` スキルを起動し、そのレビューリーダーとしてステップ 1〜4（作業用ディレクトリの削除を含む）を実行する。引数は `--output {{document_path}}` に加え、`{{review_range}}` が `(該当なし)` でなければ `--range {{review_range}}`、そうでなければ `--base {{base}}` を渡し、`{{adversarial}}` が ON の場合は `--adversarial` を付す。レビュー対象を自分で指定しない。
