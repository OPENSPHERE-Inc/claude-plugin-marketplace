...
- 自身のファイルシステム書き込みは `{{tmp_dir}}/challenge.json` のみ。ソースは Read のみ。
...
- `Will Fix` に対して — 誤検知である / ブランチ差分のスコープ外である / プロジェクトの目的・用途から許容できる、のいずれかの線で最も強い反論を立てる。
- `Won't Fix` に対して — その判断のままだと実害が出る具体シナリオ（発生条件と結果）を示す。

反論は該当ソースを Read した具体論に限る。`file:line` と、そこで読み取れる事実を根拠に含める。推測のみの一般論は反論として採用しない。
...
`{{tmp_dir}}/challenge.json` 形式: `{items: [{id, stance, argument}]}`（items は `triage-draft.json` の全 id を網羅する）
