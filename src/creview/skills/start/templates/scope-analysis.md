...
レビュアープール: 本スキルはレビュアーエージェントを同梱しない。対象プロジェクトのエージェントを `ls .claude/agents/*.md`（作業ディレクトリ基準）で列挙し、各ファイルの frontmatter の `name` / `description` を Read して各エージェントの専門性を把握する。`name` 値が別の Agent 呼び出しが `subagent_type` に渡す値である。

実施内容:

1. 差分から変更ファイルの種別 / パス / 内容領域（言語、サブシステム、ビルド / CI、A/V、コメント & FIXME / TODO 等）と拡張子別サマリを判定する。
2. 列挙した各エージェントについて、その `description` から専門性が差分に関連するか判定する。関連するエージェントはすべて、合致した拡張子 / パス / 内容領域を示す短い `reason` を付して `recommended` に追加する。
3. `.claude/agents/` ディレクトリが存在しない、空、または関連するエージェントがない場合、`{name: "general-purpose", reason: "no matching specialist agent"}` を 1 件追加する。
