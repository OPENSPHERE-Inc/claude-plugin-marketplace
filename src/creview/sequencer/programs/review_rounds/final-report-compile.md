...
全ラウンドのレビュードキュメントから最終レポートを生成する。`../../../rules/sub-agent.md`（本ファイルからの相対パスで、creview プラグインの rules/sub-agent.md）を Read し、共通禁止事項を遵守する。

入力:

- 各ラウンドのレビュードキュメント: `{{round_doc_paths}}`（例: `Round 1 → {round1_doc_path}, Round 2 → {round2_doc_path}, ...`）
- 各ラウンドの統計（参考情報）: `{{round_stats}}`（例: `Round 1: findings=N, will_fix=N, maintain=N, alternative=N, downgrade=N, fixed=N, wontfix=N, feedback_attempts=N, unresolved=N, code_changed=<bool>, ...`）
...
1. テンプレート markdown を Read して構造（`<...>` placeholder、表構造、将来推奨セクションのサブセクション例）を把握する。
...
3. テンプレートの統計サマリ・全指摘一覧・将来推奨・レビュードキュメント一覧を埋めて `{{report_path}}` に Write する。
   - 「将来の対応推奨項目」セクションへの集約規則:
     ...
     - 除外: 候補のうち、後ラウンドで同一箇所・同一内容の対応指摘が Status: 🟢 Fixed として解決済みのものは除外する（既に修正済みのため、ロードマップに残す必要がない）。同一性判定は `file:line` と指摘要旨の一致で行う。判定が困難な場合は除外せず、推奨理由欄に判定保留の旨を添える。
