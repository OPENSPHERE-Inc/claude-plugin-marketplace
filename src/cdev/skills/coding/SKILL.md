---
name: coding
description: ペアレビューセルを用いて、設計セルステップ・コーディングセルステップ・QA ゲートでコーディングタスクをエンドツーエンドに統括する、常駐エージェントチーム方式のワークフロー。architect・coder・reviewer は対象プロジェクトのエージェントから自動選定する。ユーザーが機能の実装・変更の構築・コーディングタスクの遂行を求めたとき能動的に使用する。バックグラウンドのサブエージェント（Agent の run_in_background）と命名チームメイト間メッセージング（SendMessage）が利用可能なランタイムを要する。
...
このスキルは、バックグラウンドのサブエージェント（`Agent` の `run_in_background`）と命名チームメイト宛 `SendMessage` を使用し、それらが利用可能なランタイムでのみ動作する。セッションは単一の暗黙チームを持ち、チームの明示的な作成・削除は不要。共有タスクリストは存在しない。
...
セッションは単一の暗黙チームを持つ。各 teammate は Agent ツールで `run_in_background: true` と `name` を指定して一度だけ spawn し、ステップをまたいで常駐し（コンテキストを保持）、`name` で `SendMessage` 宛先指定され、ターン間は idle になり（メッセージで起床）、完了は `SendMessage` でリーダーへ報告する。リーダーは共有タスクリストを持たず、各セルの状態を自身の作業状態で追跡する（`TodoWrite` でユーザーに可視化してよい）。

teammate 名: `architect-{slug}`、`coder-{slug}`、`reviewer-{slug}`、`dev-helper`、`comment-sensei`。

リーダーはイベント駆動で進行する: セルやタスクを dispatch したらターンを終え、teammate からのメッセージ（リーダー宛は `to: "main"`）で再起動される。各メッセージで追跡状態を更新し、ステップのゲート条件が満たされたら次のステップを開始する。
...
リーダーが保持するのは、ロスター、ペアリング、各セルの状態、重要度カウント、ファイルパス、QA 結果のみ。設計本体・ソース・指摘本体は teammate 側に留まり、指摘は producer ⇄ reviewer 間を `SendMessage` で行き来する。
...
3. `dev-helper` を `Agent(subagent_type="dev-helper", name="dev-helper", run_in_background=true, prompt=<spawn 契約>)` で spawn する。`templates/team-analysis.md` を指定して変数 `task = {タスク指定}` を渡して `SendMessage(dev-helper, ...)`。その報告（`to: "main"`）を受け取る: `{task_summary, target_languages, has_test_suite, architects:[{name, slug, scope, reviewer, reason}], coders:[{name, slug, scope, reviewer, reason}], reviewers:[{name, slug, reason}], rationale}`。各 producer の `reviewer` はペアの reviewer の `slug`（1 人の reviewer が複数の producer とペアになることもある）。
4. ロスターの各メンバーを `Agent(subagent_type={name}, name={role 名}, run_in_background=true, prompt=<spawn 契約>)` で teammate として spawn する: `architect-{slug}`、`coder-{slug}`、`reviewer-{slug}`。ロスター、ペアリング、`{task_summary}` を保持する。
5. コンソールに表示する: 各 producer のペアの reviewer と一行の理由を含むロスター。

## ステップ 2 — 設計セル（設計）

1. コンソールに表示する: `## Step 2 — Design`。
2. 各 architect について、設計セル `design-{slug}` を 2 つのメッセージで開始する:
...
1. コンソールに表示する: `## Step 3 — Coding`。`comment-sensei` を `Agent(subagent_type="comment-sensei", name="comment-sensei", run_in_background=true, prompt=<spawn 契約>)` で spawn 契約（role `the comment reviewer; a coder DMs you to review comments per templates/comment-review.md`）として spawn する（コードセルから利用可能）。
2. 各 coder について、コードセル `code-{slug}` を 2 つのメッセージで開始する:
...
3. `dev-helper` へ `templates/qa.md` を指定して `tmp_dir = {tmp_dir}`、`diff_path = {tmp_dir}/changes.txt`、`attempt_num = {n}` を渡して `SendMessage`。その報告（`to: "main"`）`{success, format_violations_fixed, workflow_source, workflow_warning, build_ran, test_ran, suggested_specialist, error_summary, summary_line}` を受け取る。`workflow_warning` が非 null の場合、ステップ 5 のために保持する。
4. `success == true` の場合、ループを抜ける。
5. `success == false` かつ試行回数が残っている場合、QA 修正セルを回す:
   a. `{suggested_specialist}` が teammate であることを保証し（チームにいなければ `Agent(subagent_type={suggested_specialist}, name="coder-{suggested_specialist}", run_in_background=true, prompt=<spawn 契約>)` で spawn する）、それとペアの reviewer を用意する（失敗スコープをカバーする reviewer、または任意の reviewer を使う）。
