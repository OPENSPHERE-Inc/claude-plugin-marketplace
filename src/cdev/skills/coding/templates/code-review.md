...
セル `{{cell_task}}` についてセルプロトコルを回す: 対応が必要な所見を `file:line` と問題内容・推奨修正方向として `{{producer}}` に DM し（`line` はファイルを Read して得た実際の行番号。文面は {{doc_lang}} で、重大度ラベルはそのまま）、重大度別の件数を 1 行（`Critical N / Major N / Minor N / Info N`）でリーダーへ報告する。
