---
name: team-analysis
description: cdev /coding ステップ 1 でコーディングタスクのスコープを定め対象プロジェクトのエージェントから architect / coder / reviewer を選定する team-analysis タスク（dev-helper）向けプロンプト
template_id: d8760930-8d32-42c1-b033-d61f0cbd19c7
---

コーディングタスクのスコープを定め、専門家チームを編成する。
...
2. プールからチームを選定し、各エージェントの `description` の専門性をタスクに合致させる:
   - architects — 設計を担う 1 体以上のエージェント。単一サブシステムのタスクなら architect 1 体で足りる。明確に分離可能な複数サブシステムにまたがる場合のみ複数を用いる。各々に `slug`（kebab-case）と `scope`（担当する設計領域）を付与する。
   - coders — 実装を担う 1 体以上のエージェント。各々に `slug`（kebab-case）と、互いに素なファイル / ディレクトリの `scope` を付与し、2 人の coder が同一ファイルを編集しないようにする。
   - reviewers — 設計とコードの双方をレビューする 1 体以上のエージェント。
3. いずれのスコープにも合致する専門家がいない役割には、その役割に `general-purpose` を 1 件用いる。
4. `task_summary` を、architects / coders が元のチャットなしで動けるタスクの自己完結的な再記述（{{doc_lang}} で）として記述する。

リーダーへの報告（SendMessage 経由）: `{task_summary, target_languages: [..], has_test_suite: <bool>, architects: [{name, slug, scope, reason}], coders: [{name, slug, scope, reason}], reviewers: [{name, reason}], rationale}`。`scope` / `reason` / `rationale` / `task_summary` は {{doc_lang}} で記述し、`name` / `slug` / 言語識別子はそのまま。TaskUpdate でタスクを完了とマークする。
