...
コメント reviewer: `{{comment_reviewer}}`
...
4. コードコメントを追加または変更した場合は、`{{comment_reviewer}}` に `{{plugin_root}}/skills/coding/templates/comment-review.md` を指定して DM する。その際 `changed_scope = {{assigned_scope}}` と `design_paths = {{design_paths}}` を渡す。comment-sensei がコメント違反を修正し、件数を報告し返す。
5. `{{reviewer}}` に変更がレビュー可能になったことを DM し、変更したファイルを列挙する。producer としてセルを回す。セルは reviewer がクローズする。
