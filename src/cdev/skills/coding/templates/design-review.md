...
セルプロトコルに従う: 対応すべき指摘（セクション / 領域、問題、推奨する修正方針。{{doc_lang}} で、重要度ラベルはそのまま）を `{{producer}}` へ DM し、重要度別の件数 `{critical, major, minor, info}` を JSON 文字列でリーダーへ報告する。producer の triage 後に resolve し、セル `{{cell_task}}` の resolve を `SendMessage(to: "main")` でリーダーへ報告する。なお同意できない却下された `Critical` はエスカレーションする。
