...
- 変更スコープ: `{{changed_scope}}`（coder が編集したファイル / ディレクトリ）。そこで追加・変更されたコメントを、追跡済みファイルと未追跡ファイルの双方について抽出する。`git diff` は未追跡ファイルを出力しないため、新規ファイルのコメントは全体が「追加」であり別途拾う必要がある。コメント記号は言語別（`//` / `#` / `/* */` / `<!-- -->` 等）。
