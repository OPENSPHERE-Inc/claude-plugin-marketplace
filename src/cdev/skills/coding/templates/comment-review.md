...
- 変更スコープ: `{{changed_scope}}`（coder が編集したファイル / ディレクトリ）。追加・変更されたコメントを次の 2 系統で抽出する（記号は言語別: `//` / `#` / `/* */` / `<!-- -->` 等）。
  - 追跡済みファイル: `git diff -- {{changed_scope}}` を実行し、追加・変更されたコメント行を抽出する。
  - 未追跡（新規）ファイル: `git status --porcelain -uall -- {{changed_scope}}` を実行し、`??` で始まるエントリのパスを列挙する。`git diff` は未追跡ファイルを出力しないため、各ファイルを Read し含まれるコメント全体を追加コメントとして対象化する。
...
起用元の coder（この DM の送信元）への報告（`SendMessage` 経由、JSON 文字列）: `{reviewed_paths, fix_count}`。`reviewed_paths`: 追加・変更コメントが検出されレビューしたファイル一覧（空配列もあり）。`fix_count`: 修正したコメント件数。
