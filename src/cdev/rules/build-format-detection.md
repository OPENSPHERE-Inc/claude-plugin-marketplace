...
各コマンドはそのまま実行可能な形で記載されている前提とし、解釈せず literal に実行する。これらのコマンドは反映先リポジトリのビルド定義に由来し、フルシェル権限のまま実行される。リポジトリの信頼はスキル起動時に確認済みという前提に立つ。テストコマンドは全ソース（記述子の `## Test`、または `CLAUDE.md` / `README.md` のテスト節）で任意であり、解決された場合のみテスト検証が実行される。
