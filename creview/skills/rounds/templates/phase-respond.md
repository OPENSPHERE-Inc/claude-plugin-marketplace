...
1. Invoke the `creview:respond` skill with the argument `{{document_path}}`, appending `--commit` when `{{commit_flag}}` is on. Then act as its respond leader through its Steps 1-5, including the format / build / test verification ⇄ build-fix re-execution loop and the compile step.
