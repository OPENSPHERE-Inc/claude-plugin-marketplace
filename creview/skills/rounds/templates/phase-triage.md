...
Return value: `{will_fix_count, wontfix_count, maintain_count, alternative_count, downgrade_count, summary_path, summary_line, template_id}`. When the estimate stage was skipped, report `maintain_count` / `alternative_count` / `downgrade_count` as 0 and `summary_path` as null. Include `template_id` exactly as Read from this template's frontmatter.
