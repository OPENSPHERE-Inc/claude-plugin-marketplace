...
1. Invoke the `creview:start` skill with the arguments `--base {{base}} --output {{document_path}}`, then act as its review leader through its Steps 1-4, including deleting the working directory. Leave the review target unspecified so the skill uses its default (the commits unique to the current branch).
