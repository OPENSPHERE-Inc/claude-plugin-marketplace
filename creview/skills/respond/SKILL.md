...
- Relation to build verification: commit after Step 3 (build verification). If a build error occurs, include the build fix in the commit as well.
...
- `status` (Step 3) — Value format: `🟢 Fixed — {concise description of the fix}`.
...
5. If the build still fails after the maximum attempts, present `error_summary` to the user, exit the loop, and proceed to Step 4.
...
## Step 4 — Reflect into the review document (delegate to the aggregator sub-agent)
