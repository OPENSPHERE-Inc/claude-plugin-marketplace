...
{tmp_dir}/triage-draft.json    ← primary decisions (triage sub-agent)
{tmp_dir}/challenge-{n}.json   ← outputs of the three challenge sub-agents (spawned by the triage Sub)
{tmp_dir}/adjudication.json    ← output of the adjudication sub-agent (spawned by the triage Sub)
{tmp_dir}/triage.json          ← final decisions (triage sub-agent)
...
The markdown reflection uses `{tmp_dir}/events.jsonl`, generated by `compile-review.py` from the `memo_value` of the intermediate JSON (triage.json / estimates/*.json). Format:
