...
- `estimate` (Step 2) — Value format: `▶️ Maintain — Cost: {S/M/L}, Future: {S/M/L}, Signals: {none\|a,b,c,d,e,f}` / `🔻 Downgrade — Cost: ..., Future: ..., Signals: ... — {downgrade reason}` / `🚧 Alternative — Cost: ..., Future: ..., Signals: ... — FIXME insertion: {direction}`.
