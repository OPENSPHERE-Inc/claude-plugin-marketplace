...
- Keep the draft `verdict` and `reason` when the objection lacks concreteness, and when weighing the objection against the draft's basis leaves the matter unsettled. `stance: no_valid_objection` also keeps the draft.
