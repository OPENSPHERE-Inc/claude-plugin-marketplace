...
常時有効で、トリアージサブエージェントの一次判定に 3 体の反証サブエージェントが
それぞれ独立に可能な範囲で反対の論を立て、裁定サブエージェントが最終判定を
決めます。一次判定が覆るのは、3 体のうち 2 体以上が反転を支持し、かつ裁定
サブエージェントがその根拠となる事実を確認できた場合に限られます。
...
- `/creview:rounds` — 3 以上。各フェーズをフェーズリーダーサブエージェントで
  ...
  トリアージサブエージェントがさらに反証サブエージェント 3 体と裁定
  サブエージェントを起動します。
- `/creview:triage` 単体 — 2 以上。トリアージサブエージェントが反証
  サブエージェント 3 体と裁定サブエージェントを起動します。
