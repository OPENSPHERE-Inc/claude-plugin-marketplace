...
Each command is assumed to be written in a directly executable form; execute it literally without interpretation. These commands originate from the destination repository's build definitions and run with full shell privileges under a trusted-repository assumption, so do not run cdev coding against an untrusted repository. The test command is optional in every source (a descriptor `## Test` section, or a test section in `CLAUDE.md` / `README.md`); test verification runs only when one is resolved.
