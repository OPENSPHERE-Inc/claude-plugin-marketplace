...
- **Reviewer**: review the output; DM each actionable (Critical / Major) finding — location, issue, recommended fix direction — to the producer; report severity counts to the leader. After the producer triages, resolve: verify the fixes are adequate and the rejections reasonable. When satisfied, mark the cell task done via `TaskUpdate` and tell the leader the cell is resolved. Repeat review ⇄ triage up to the round cap the leader gave.
