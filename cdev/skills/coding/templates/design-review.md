...
Per the cell protocol: DM actionable findings (the section / area, the issue, the recommended fix direction; in {{doc_lang}}, severity labels as-is) to `{{producer}}`; report severity counts `{critical, major, minor, info}` to the leader; resolve after the producer triages; mark `{{cell_task}}` done via TaskUpdate; and escalate a rejected `Critical` you still disagree with.
