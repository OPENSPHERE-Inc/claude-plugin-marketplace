...
4. If you added or modified any code comment, DM `comment-sensei` naming `{{plugin_root}}/skills/coding/templates/comment-review.md` with `changed_scope = {{assigned_scope}}` and `design_paths = {{design_paths}}`; it fixes comment violations and reports the count back to you.
